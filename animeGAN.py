...
# generate new kinds of anime characters 
...
def generator(input, random_dim, is_train, reuse=False):
    ...
    with tf.variable_scope('gen') as scope:
        ...
        w1 = tf.get_variable('w1', shape=[random_dim, s4 * s4 * c8], dtype=tf.float32,
                             initializer=tf.truncated_normal_initializer(stddev=0.02))
        b1 = tf.get_variable('b1', shape=[c8 * s4 * s4], dtype=tf.float32,
                             initializer=tf.constant_initializer(0.0))
        flat_conv1 = tf.add(tf.matmul(input, w1), b1, name='flat_conv1')
         #Convolution, bias, activation, repeat! 
        conv1 = tf.reshape(flat_conv1, shape=[-1, s4, s4, c8], name='conv1')
...
def discriminator(input, is_train, reuse=False):
    c1, c2, c4, c8""", c16""" = 32, 64, 128, 256 """, 512 """ # channel num: 64, 128, 256, 512
    with tf.variable_scope('dis') as scope:
        ...
        #Convolution, activation, bias, repeat! 
        conv3 = tf.layers.conv2d(act2, c4, kernel_size=[5, 5], strides=[2, 2], padding="SAME",
                                 kernel_initializer=tf.truncated_normal_initializer(stddev=0.02),
                                 name='conv3')
        bn3 = tf.contrib.layers.batch_norm(conv3, is_training=is_train, epsilon=1e-5, decay = 0.9,  updates_collections=None, scope='bn3')
        act3 = lrelu(bn3, n='act3')
         #Convolution, activation, bias, repeat! 
        conv4 = tf.layers.conv2d(act3, c8, kernel_size=[5, 5], strides=[2, 2], padding="SAME",
                                 kernel_initializer=tf.truncated_normal_initializer(stddev=0.02),
                                 name='conv4')
        bn4 = tf.contrib.layers.batch_norm(conv4, is_training=is_train, epsilon=1e-5, decay = 0.9,  updates_collections=None, scope='bn4')
